...
Net485::Net485()
{
  ...
  this->procCallbackOnReceive = NULL;
  this->procCallbackOnReceiveBroadcast = NULL;
}
  ...
  unsigned short bufferSize)
{
  ...
  pinMode(this->enableTxPin, OUTPUT);
  digitalWrite(this->enableTxPin, LOW);

  setTimeoutRX(this->timeoutRX);
...
void Net485::setTimeoutRX(unsigned int timeoutRX) {
  ...
  Serial.setTimeout(this->timeoutRX);
}
  ...
  if ( readRequest() == RetValidPack ) {
    ...
    if (this->addrDestination == address) {
      ...
      if ( procCallbackOnReceive != NULL ) {
        procCallbackOnReceive(event, buffer, length);
      }
    } else if (addrDestination == AddrBroadcast ) {
      if ( procCallbackOnReceiveBroadcast != NULL ) {
        procCallbackOnReceiveBroadcast(event, buffer, length);
      }
    }
...
{
  digitalWrite(this->enableTxPin, HIGH);

  //----------------------

  Serial.write(ByteSYNC);
  Serial.write(ByteSOH);
  Serial.write(this->method);
  Serial.write(this->addrSource);
  Serial.write(this->addrDestination);
  Serial.write(this->event);
  Serial.write(this->response);

  Serial.flush();

  this->status.counterBytesTX += 7;
  ...
  unsigned short length)
{
  ...
  Serial.write(LOBYTE(this->length));
  CRC16Byte(&crc, LOBYTE(this->length));
  Serial.write(HIBYTE(this->length));
  CRC16Byte(&crc, HIBYTE(this->length));

  if ( this->length > 0 )
  {
    Serial.write(ByteSTX);
    CRC16Byte(&crc, ByteSTX);

    for ( int x = 0; x < this->length; x++ ) {
      Serial.write(buffer[x]);
      CRC16Byte(&crc, buffer[x]);
    }

    Serial.write(ByteETX);
    CRC16Byte(&crc, ByteETX);
  }

  Serial.write(LOBYTE(crc));
  Serial.write(HIBYTE(crc));

  Serial.write(ByteEOT);

  Serial.flush();

  //----------------------

  this->status.counterBytesTX += 7 + this->length;
...
unsigned char Net485::writeResponse()
{
  ...
  int received = Serial.readBytes((char*)buffer, 7);
